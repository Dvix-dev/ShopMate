...
13. 🟢 **Próxima prioridad: §1.D Control de items (editar/borrar)**. Roadmap actualizado 2026-07-13. David detectó que ahora que "Validar compra" archiva al historial (§2.A), la única forma de corregir un item es validándolo, lo cual ensucia el historial con compras "fantasma". Antes de §1.E Familias (renumerada), hay que añadir CRUD básico: editar nombre inline, borrar con 🗑️ + undo, restricciones de edición (1..80 chars), a11y teclado, target táctil 44px mobile. RTDB rules ya validan nombre, no hay que tocar `database.rules.json`. Pendiente de implementación en próxima sesión.
14. 🟡 **Cuota Spark de Firebase Auth (2026-07-13)**: el plan Spark (gratuito) tiene un **límite diario de envios de email** para `sendSignInLinkToEmail`. Cuando se agota, Firebase devuelve `auth/quota-exceeded` (HTTP 400). NO es un bug de código, es limitación del plan. **Reset**: a medianoche hora del Pacifico (UTC-8/UTC-7 según DST). **Workaround**: upgrade a Blaze (pay-as-you-go) en Firebase Console. El error message en `AUTH_ERROR_MAP['auth/quota-exceeded']` ahora explica todo esto al user (commit fix(auth-ux) 2026-07-13). Si David reporta "no me deja hacer login" y el log muestra `auth/quota-exceeded`, es esto, no un bug.
15. 🟢 **Emulador auto-sign-in como "test" (2026-07-13)**: cuando se usa `?env=emul`, la app **YA NO muestra la auth modal ni pide email-link**. Hace `signInAnonymously(auth)` + `updateProfile({displayName: 'test'})` automaticamente desde el callback `onAuthStateChanged` cuando `user === null && IS_EMULATOR` (helper `emulatorSignInTest()`). El UID sigue siendo random (Firebase lo asigna en cada arranque limpio), pero el nombre visible en la UI (drawer perfil, log de consola) es "test". **Sin contrasena** (es signInAnonymously), **sin quota** (no usa sendSignInLinkToEmail), **sin email-link**. El safety net de 5s `authFallbackTimer` se cancela en emulador (`if (IS_EMULATOR) return`) para no romper el flujo "entra directo". En prod (sin `?env=emul`) el comportamiento NO cambia: sigue pidiendo email magic link. Si quieres probar el flow email-link "real" en local, desactiva temporalmente la rama `IS_EMULATOR` en el bloque `if (IS_EMULATOR) { ... }` del callback `onAuthStateChanged`, o usa el UI del emulador (http://127.0.0.1:4000/auth) para crear usuarios manualmente. La constante `IS_EMULATOR` se declara arriba del todo (antes de los imports Firebase) y se evalua una sola vez al cargar el script.
